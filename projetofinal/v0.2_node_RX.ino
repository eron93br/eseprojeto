 ...
 * 
 *             ======== CARRO PARADO, ELE APITA O BUZZER QUANDO ESTÁ MENOR QUE 30M ============
 * 
 *-------------------------------------------------------------
...
void loop()
{
  ...
  if (rf95.available())
  {
    ...
    if (rf95.recv(buf, &len))
    {
      ...
      distancia = gps.distanceBetween(lat_carNode1, lng_carNode1,  -8.049830, -34.961550);
      // --------------------------------------------------------
      Serial.println(distancia);  //Serial.println(buf[1]); Serial.println(buf[2]); Serial.println(buf[3]);
    }
    else
    {
